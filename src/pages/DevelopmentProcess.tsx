      ...
      <section className="pt-8 pb-16">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-4xl mx-auto">
            <Link to="/" className="flex items-center text-gray-400 hover:text-white mb-8">
            ...
            <div className="prose prose-lg max-w-none">
              ...
              opacity: 0
                ...
                <div className="hidden md:block absolute top-0 left-1/2 w-0.5 h-full bg-zinc-700 -translate-x-1/2"></div>
                ...
                <div className="space-y-10 relative">
                  {processes.map((process, index) => <div key={process.id} ref={el => processSectionsRef.current[index] = el} className={cn("relative flex flex-col md:flex-row md:items-center gap-6", index % 2 === 0 ? "md:flex-row" : "md:flex-row-reverse text-right")}>
                      <div className="md:w-1/2">
                        <div className={cn("md:absolute top-0 left-1/2 md:-translate-x-1/2 w-12 h-12 rounded-full flex items-center justify-center z-10 transition-all duration-300", activeProcess === process.id ? "bg-[#cf4500] text-white scale-110" : "bg-zinc-900 text-gray-400 border border-zinc-700")} onClick={() => setActiveProcess(process.id)}>
                          <span className="font-bold">{process.id}</span>
                        </div>
                        
                        <h3 className="text-xl font-bold mb-2 mt-3 md:mt-0 text-white">{process.title}</h3>
                        <p className="text-gray-300 mb-3 text-sm">{process.description}</p>
                        
                        <button onClick={() => setActiveProcess(process.id)} className={cn("text-sm font-medium transition-colors", activeProcess === process.id ? "text-white" : "text-gray-400 hover:text-white")}>
                          {activeProcess === process.id ? "Currently Viewing" : "View Details"}
                        </button>
                      </div>
                      
                      <div className={cn("md:w-1/2 bg-[#0f0f0f] rounded-xl p-5 shadow-sm border border-zinc-800 transition-all duration-300", activeProcess === process.id ? "opacity-100 translate-y-0 shadow-md border-zinc-700" : "opacity-50 md:opacity-30 hover:opacity-70 cursor-pointer")} onClick={() => setActiveProcess(process.id)}>
                        ...
                        <ul className="space-y-2">
                          {process.steps.map((step, stepIndex) => <li key={stepIndex} className="flex items-start">
                              <span className="flex-shrink-0 w-5 h-5 rounded-full bg-zinc-800 flex items-center justify-center mt-0.5 mr-2">
                                <Check className="w-3 h-3 text-[#cf4500]" />
                              </span>
                              <span className="text-gray-300 text-sm text-left">{step}</span>
                            </li>)}
                        </ul>
                      </div>
                    </div>)}
              ...
              <div className="bg-[#0f0f0f] p-8 rounded-lg my-12 border border-zinc-800">
                <h3 className="text-xl font-semibold mb-4 text-white">Our Development Principles</h3>
            ...
            <div className="mt-12 pt-8 border-t border-zinc-800">
              <Link to="/tech-details" className="inline-flex items-center px-6 py-3 bg-[#cf4500] text-white rounded-lg hover:bg-[#ff5a1a] transition-all group">
